...
const VendorDashboard = () => {
  ...
  const [dashboardData] = useState({
    ...
    servicePerformance: [
      { name: "Wedding Catering", percentage: 65, color: "#d7a444" },
      { name: "Corporate Buffets", percentage: 28, color: "#708aa0" },
      { name: "Private Parties", percentage: 7, color: "#0b2d49" }
    ],
    recentActivity: [
      {
        id: 1,
        ...
        status: "ACCEPTED",
        details: "Confirmed for Dec 15, 2023. Manager: Alex Rivera.",
        time: "2 hours ago",
        type: "booking"
      },
      {
    ...
    upcomingConsultations: [
      { id: 1, name: "Sarah Jenkins", time: "Tomorrow at 10:00 AM", type: "video" },
      { id: 2, name: "Michael Chen", time: "Oct 28 at 2:30 PM", type: "chat" }
    ]
  });
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-12 lg:col-span-4 space-y-8">
                ...
                <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-[#708aa0]/5">
                    <h3 className="text-xl font-black mb-8">Upcoming Consultations</h3>
                    <div className="space-y-4">
                        {dashboardData.upcomingConsultations.map((item) => (
                            <div key={item.id} className="p-4 bg-[#f3ddb1]/20 rounded-2xl flex items-center justify-between group hover:bg-[#f3ddb1]/40 transition-all border border-[#f3ddb1]/30 hover:border-[#d7a444]/40 cursor-pointer shadow-sm hover:shadow-md">
                                <div className="flex items-center gap-4">
                                    <div className="p-3 bg-white rounded-xl text-[#d7a444] shadow-sm group-hover:scale-105 transition-transform">
                                        {item.type === 'video' ? <BsGrid /> : <BsChatDots />}
                                    </div>
                                    <div>
                                        <p className="font-black text-sm">{item.name}</p>
                                        <p className="text-[10px] text-[#5a5b44] font-bold uppercase tracking-tight leading-none">{item.time}</p>
                                    </div>
                                </div>
                                <button className="p-2 text-[#d7a444] hover:scale-110 transition-transform">
                    ...
                    <button 
                        onClick={() => toast("Loading all consultations...")}
                        className="w-full mt-8 py-4 border-2 border-[#e9eff1] rounded-2xl text-sm font-black text-[#0b2d49] hover:bg-[#0b2d49]/5 transition-all shadow-sm active:scale-95"
                    >
