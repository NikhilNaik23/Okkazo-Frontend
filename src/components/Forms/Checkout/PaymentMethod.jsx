...
import { BsCheckCircleFill, BsShieldCheck, BsLightningCharge, BsWallet2, BsArrowRight } from "react-icons/bs";

const PaymentMethod = ({ onConfirm, platformFee }) => {
    // Simplified selection for Razorpay flow
    const [selectedMethod, setSelectedMethod] = useState("all");

    const paymentOptions = [
        { id: "all", name: "Cards, UPI, NetBanking", desc: "All-in-one checkout via Razorpay", icon: <BsLightningCharge /> },
        { id: "upi", name: "UPI (Google Pay, PhonePe)", desc: "Direct bank transfer using UPI ID", icon: <BsWallet2 /> },
    ];
        ...
        <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-[0_20px_50px_-10px_rgba(9,99,126,0.1)] border border-[#09637E]/5 h-full relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-12 relative z-10">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-black text-[#09637E]/40 uppercase tracking-[0.2em] mt-1">Powered by Razorpay</p>
                    </div>
                </div>
            ...
            <div className="space-y-4 mb-12 relative z-10">
                ...
                {paymentOptions.map((option) => (
                    <div
                        ...
                        className={`group cursor-pointer p-6 rounded-3xl border-2 transition-all duration-300 flex items-center justify-between ${selectedMethod === option.id
                            ? "bg-[#09637E]/5 border-[#09637E] shadow-lg shadow-[#09637E]/5"
                            : "bg-transparent border-[#09637E]/10 hover:border-[#09637E]/30"
                            }`}
                    >
                        <div className="flex items-center gap-5">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${selectedMethod === option.id ? "bg-[#09637E] text-white" : "bg-[#EBF4F6] text-[#09637E]"
                                }`}>
                                {option.icon}
                            </div>
                            <div>
                                <h4 className={`font-bold transition-colors ${selectedMethod === option.id ? "text-[#09637E]" : "text-[#09637E]/80"}`}>
                                    {option.name}
                                </h4>
                                <p className="text-xs text-[#09637E]/50 font-medium">{option.desc}</p>
                            </div>
                        </div>
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${selectedMethod === option.id ? "bg-[#22c55e] border-[#22c55e]" : "border-[#09637E]/20"
                            }`}>
                            {selectedMethod === option.id && <BsCheckCircleFill className="text-white text-xs" />}
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="relative z-10">
                ...
                >
                    ...
                    <span className="font-black uppercase tracking-[0.3em] text-xs relative z-10">Proceed to Razorpay</span>
                ...
                <div className="flex flex-col items-center gap-3 mt-8">
                    <div className="flex items-center gap-2 text-[9px] font-bold text-[#09637E]/30 uppercase tracking-[0.2em]">
                        ...
                        Your transaction is protected by Razorpay Security
                    </div>
                </div>
            </div>
        </div>
